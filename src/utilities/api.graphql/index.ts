...
const supabaseGraphqlClient = axios.create({
    // todo: capture CLIENT_HOSTNAME when using the setup script
    baseURL: (paths.supabaseGraphqlHost),
    headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
    },
...
const frameworkGraphqlClient = axios.create({
    // todo: capture CLIENT_HOSTNAME when using the setup script
    baseURL: (paths.frameworkGraphqlHost),
    headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
    },
...
const queries = ({
     ...
     * @returns {import("react-query").UseQueryOptions} An object suitable for use with the `useQuery` hook
     */
    graphQuery: (query: string, server: string | number) => ({
        queryKey: ["graphql", query, server],
        queryFn: async () => ({
            4100: supabaseGraphqlClient.post("/", {query}),
            4200: frameworkGraphqlClient.post("/", {query}),
            4101: graphqlClient.post("/", {query}),
        }[server])
    }),
});
